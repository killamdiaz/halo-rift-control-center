...
const AddDeviceModal: React.FC<AddDeviceModalProps> = ({ open, onOpenChange }) => {
  ...
  const handlePair = async (device: ScannedDevice) => {
    ...
    const role = type === 'shoe'
      ? (device.name.toLowerCase().includes('left') ? 'left' : 'right')
      : 'primary';
    try {
            ...
            {scannedDevices.map((device, index) => (
              ...
              >
                <div className="flex items-center">
                  {device.type?.toLowerCase() === 'shoe' && <Footprints className="w-5 h-5 text-halo-accent mr-2" />}
                  {device.type?.toLowerCase() === 'gun' && <Gamepad2 className="w-5 h-5 text-halo-accent mr-2" />}
                  {device.type?.toLowerCase() === 'suit' && <ShirtIcon className="w-5 h-5 text-halo-accent mr-2" />}
          ...
          {pairedDevices.length > 0 && (
            <div className="mt-6 w-full max-w-md">
              ...
              {pairedDevices.map((device, index) => (
                ...
                >
                  <div className="flex items-center">
                    {device.type?.toLowerCase() === 'shoe' && <Footprints className="w-5 h-5 text-green-400 mr-2" />}
                    {device.type?.toLowerCase() === 'gun' && <Gamepad2 className="w-5 h-5 text-green-400 mr-2" />}
                    {device.type?.toLowerCase() === 'suit' && <ShirtIcon className="w-5 h-5 text-green-400 mr-2" />}
